...
$l = array(
	array('fuse','html','Logout','session_terminate','logout',false),
	array('fuse','xml','Security XML','_xml','xml/security',false)
);
...
while ($c = array_shift($l)) {
	...
	if (is_string($id)&&(!is_null($c[4]))) { $t->cms->create_path(array('path'=>$c[4],'element_id'=>$id,'menu_setting'=>$c[5],'force'=>true)); }
}
